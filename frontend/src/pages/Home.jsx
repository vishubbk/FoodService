...
const Home = () => {
  ...
  const fetchPage = useCallback(async (p = 1) => {
    try {
      ...
      const res = await axios.get(`${API_BASE}/api/user/getAllItems?page=${p}&limit=${limit}`, {
  ...
  useEffect(() => {
    ...
    const options = { root: containerRef.current, rootMargin: "300px 0px", threshold: 0.5 };
  ...
  useEffect(() => {
    ...
    if (!hasMore) return;
    const opt = { root: containerRef.current, rootMargin: "0px", threshold: 0.3 };
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
        <div ref={containerRef} className="h-full overflow-y-scroll snap-y snap-mandatory" style={{ WebkitOverflowScrolling: "touch" }}>
          {loading ? (
            <div className="h-screen flex items-center justify-center">Loading...</div>
          ) : error ? (
            <div className="h-screen flex items-center justify-center text-red-600">{error}</div>
          ) : items.length === 0 ? (
            <div className="h-screen flex items-center justify-center">No items found.</div>
          ) : (
            items.map((item) => (
              <div key={item._id || item.id} className="reel-item snap-start h-screen w-full flex flex-col bg-black">
                <div className="relative flex-1 w-full flex items-center justify-center">
                  {item.video ? (
                    <video
                      ref={(el) => { if (el) videoRefs.current.set(item._id || item.id, el); else videoRefs.current.delete(item._id || item.id); }}
                      data-src={item.video}
                      playsInline
                      muted
                      controls={false}
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gray-800 text-white">No preview</div>
                  )}

                  <div className="absolute bottom-6 left-4 text-white">
                    <h3 className="font-semibold text-lg">{item.name || "Untitled"}</h3>
          ...
          <div ref={sentinelRef} className="h-24 flex items-center justify-center">{loadingMore ? <span>Loading more...</span> : hasMore ? <span>Scroll to load more</span> : <span>No more items</span>}</div>
        </div>
      </div>
    </div>
  );
